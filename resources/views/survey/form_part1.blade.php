...
@section('title', 'แบบสอบถามตอนที่ 1')
...
<div class="min-h-screen pt-24 pb-12 bg-slate-50 font-sans">
    <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div class="flex items-center justify-between mb-8">
            {{-- ปุ่มย้อนกลับ (กลับไปหน้า Part 0) --}}
            <a href="{{ route('survey.form', ['id' => $control->ID]) }}" 
        ...
        <form action="{{ route('survey.storePart1') }}" method="POST">
            @csrf

            <input type="hidden" name="id" value="{{ $control->ID }}">

            <div class="space-y-6">
                <x-card title="ข้อ 1: รูปแบบ..." class="!overflow-visible">
                </x-card>
            </div>
